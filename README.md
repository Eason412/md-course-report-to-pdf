...
- 参考文献另页开始，标题居中。
- 可选地通过 Markdown 顶部 front-matter 生成学位论文封面（官方附件 2.1 版式），含分类号、密级、UDC、学位类型、题名/副题名、作者、指导教师及职称、学位类别、学科/专业名称、研究方向、论文提交时间等字段；无这些字段时使用课程报告封面，行为不变。
...
| 字段 | 含义 | 示例 |
|---|---|---|
| `cover` | 封面类型，填 `thesis` 强制使用学位论文封面；出现下列任一学位字段时也会自动切换 | `thesis` |
...
留空的字段在封面上显示为空下划线。命令行参数（如 `--course`、`--student-name`）若同时提供，会覆盖 front-matter 中对应的值。若 Markdown 中不含任何学位字段，则自动回退到课程报告封面，行为与之前完全一致。
